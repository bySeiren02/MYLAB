...
const YearlyGoal = () => {
  ...
  const loadGoals = () => {
    const data = getStorage(`yearly_goals_${currentYear}`, [])
    setGoals(data)
  }

  const saveGoals = (updatedGoals) => {
    setStorage(`yearly_goals_${currentYear}`, updatedGoals)
  ...
  return (
    <div className="base-page">
      ...
      <div className="page-header">
        <h1 className="page-title">{t('yearlyGoal')}</h1>
      </div>

      <div className="date-selector" style={{ marginBottom: '20px' }}>
        <button className="date-btn" onClick={goToPreviousYear}>
          &lt;
        </button>
        <div className="date-display">{currentYear}{language === 'ko' ? '년' : ''}</div>
        <button className="date-btn" onClick={goToNextYear}>
          &gt;
        </button>
      </div>
